...
# === USER INPUTS ===
input_nc_file = "/home/duch/shipping/monthdailysum/daysumapr2023.nc"  # change for each month
gspro_file = "/home/duch/shipping/speciate/S2S-Tool/output/gspro.CB6R3_AE7_CRITERIA_VOC.CMAQ.2025-06-29.txt"
profile_id = "121GROC"  # the emission profile used for ship emissions
output_nc_file = input_nc_file.replace(".nc", "_cb6r3_speciated.nc")
...
# === STEP 3: CREATE NEW DATASET WITH SPECIATED FIELDS ===
ds_out = xr.Dataset()
...
# === COPY COORDINATES AND TIME FROM ORIGINAL DATASET ===
for coord in ["time", "easting", "northing"]:
    if coord in ds:
        ds_out[coord] = ds[coord]

# === STEP 4: SAVE OUTPUT FILE ===
ds_out.to_netcdf(output_nc_file)
print(f"Speciated emissions written to: {output_nc_file}")
